...
const TicketDetailSchema = t.Object({
  id: t.String(),
  title: t.String(),
  description: t.String(),
  status: t.String(),
...
export const ticketsRouter = new Elysia({ tags: ['Tickets'] })
  ...
  .get('/api/tickets', async ({ query, authUser }) => {
    ...
    detail: {
      ...
      description: 'Cursor-based paginated ticket list. Soft-deleted tickets excluded. Supports filtering by status, priority, assignee, reporter, or "mine".',
      security: [{ cookieAuth: [] }],
      responses: {
    ...
    query: t.Object({
      ...
      status: t.Optional(t.Union([
        t.Literal('OPEN'), t.Literal('IN_PROGRESS'), t.Literal('READY_FOR_QC'),
        t.Literal('REOPENED'), t.Literal('CLOSED'),
  ...
  .post('/api/tickets', async ({ body, set, authUser, request }) => {
    const guard = guardQcOrAdmin(authUser); if (guard) return guard
  ...
  .patch('/api/tickets/:id', async ({ body, params, set, authUser, request }) => {
    const guard = guardQcOrAdmin(authUser); if (guard) return guard
...
- **ADMIN / SUPER_ADMIN:** OPEN→IN_PROGRESS, IN_PROGRESS→READY_FOR_QC, REOPENED→IN_PROGRESS`,
  ...
  .post('/api/tickets/:id/comments', async ({ body, params, set, authUser }) => {
    const guard = guardQcOrAdmin(authUser); if (guard) return guard
    ...
    detail: {
      ...
      description: 'Adds a comment. The authorTag is set automatically based on the user\'s role (QC / ADMIN / SUPER_ADMIN).',
      security: [{ cookieAuth: [] }],
      responses: {
  ...
  .post('/api/tickets/:id/evidence', async ({ body, params, set, authUser }) => {
    const guard = guardQcOrAdmin(authUser); if (guard) return guard
    const ticket = await prisma.ticket.findFirst({ where: { id: params.id, ...notDeleted }, select: { id: true } })
    if (!ticket) { set.status = 404; return { error: 'Ticket not found' } }
    ...
    body: t.Object({
      kind: t.Union([
        t.Literal('screenshot'), t.Literal('commit'), t.Literal('test_log'),
        t.Literal('trace'), t.Literal('other'),
