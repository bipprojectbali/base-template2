...
// Threshold sesuai docs/FILE-HEALTH.md
const CATEGORY_RULES: CategoryRule[] = [
  // Test files first (before backend match for src/* would catch them)
  { category: 'test', match: (p) => p.startsWith('tests/'), limitLines: 400, limitChars: 16_000 },
  // Frontend routes (page orchestrators) — pakai hard global limit 500/20000
  {
  ...
  // Backend lib (services, helpers, utils)
  { category: 'lib', match: (p) => p.startsWith('src/lib/'), limitLines: 250, limitChars: 10_000 },
  // Other backend (entry points, app.ts) — config-tier
...
// Files yang dikecualikan dari aturan
...
export const adminFileHealthRouter = new Elysia({ tags: ['Admin — Info'] })
  ...
  .get(
    ...
    async ({ authUser }) => {
      ...
      // Sort by worst ratio (highest first), exempt last
      files.sort((a, b) => {
        if (a.exempt !== b.exempt) return a.exempt ? 1 : -1
        const aRatio = Math.max(a.ratioLines, a.ratioChars)
        const bRatio = Math.max(b.ratioLines, b.ratioChars)
      ...
      const worstOffenders = files.filter((f) => !f.exempt).slice(0, 10)
    ...
    },
    {
      detail: {
        ...
        description:
          'Scans project files across src/, prisma/, tests/, scripts/, docs/ and reports lines, characters, and status (ok/warn/critical/exempt) against the limits defined in docs/FILE-HEALTH.md. Use this to detect files that need to be split before they balloon.',
        security: [{ cookieAuth: [] }],
        responses: {
