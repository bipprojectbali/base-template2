...
export const adminUsersRouter = new Elysia({ tags: ['Admin — Users'] })
  ...
  .get('/api/admin/users', async ({ authUser }) => {
  ...
  .put('/api/admin/users/:id/role', async ({ request, params, set, authUser, body }) => {
    const guard = guardSuperAdmin(authUser); if (guard) return guard
  ...
  .put('/api/admin/users/:id/block', async ({ request, params, set, authUser, body }) => {
    const guard = guardSuperAdmin(authUser); if (guard) return guard
    const ip = getIp(request)
    if (authUser!.id === params.id) {
      set.status = 400
